...
Poor local representation can trick optimisation gradient to the wrong direction, especially when the direction pointing to local minimum contradicts to global minimum, which It can be challenge when exploring $\mathcal{Obj}(\phi)$ space, even more so if encounters poor local representation. 
\end{enumerate}

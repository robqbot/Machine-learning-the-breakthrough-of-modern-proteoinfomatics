...
Poor local representation can trick optimisation gradient to the wrong direction, especially when the direction of local minimum contradicts to global minimum, which in turn deterirate It can be challenge when exploring $\mathcal{Obj}(\phi)$ space, even more so if encounters poor local representation. 
\end{enumerate}

...
\begin{equation}
    ...
    \mathcal{Obj}(\phi)]
\end{equation}
where $\mathcal{V}$ is the average of the gradients $g = \mathcal{H}_{\phi} [\frac{1}{m} \sum_{i=1}^m \mathcal{Obj}(\phi)]$ across training set with $m$ training samples, $\lambda$ is a hyper-parameter with range $[0,1)$ and defines the weight of last gradient of the 
\end{enumerate}

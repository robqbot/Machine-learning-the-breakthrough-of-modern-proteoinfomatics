...
\end{equation}
In the context of machine learning, Hessian matrix is often real and symmetric. In multi-dimensional input evaluation, the curvature at direction $\mathcal{u}$ is given by $\mathcal{u}^T\mathcal{H}\mathcal{u}$. Therefore, the evaluation of $f(x)$ at $x$ using second-order Taylor appro
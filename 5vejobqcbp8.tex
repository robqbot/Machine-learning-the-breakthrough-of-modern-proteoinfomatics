...
\begin{equation}
    ...
    \mathcal{Obj}(\phi)]
\end{equation}
The step update is provided by $\phi_{i+1} = \phi_i +$ where $\mathcal{V}$ is the sum of the gradient terms $g = \mathcal{H}_{\phi} [\frac{1}{m} \sum_{i=1}^m \mathcal{Obj}(\phi)]$ across training set with $m$ training samples. $\lambda$ is a hyper-parameter with range $[0,1)$ and defines the weight of  gradient terms on the decay, where a bigger $\lambda$ relative to $\mu$, a stronger weight of gradient. 
\end{enumerate}

...
\end{align}
where $H$ is the Hessian matrix for $Obj$ at $\phi_i$. As long as $\mathcal{Obj}$ is convex, model can be trained iteratively. For the non-convex $Obj$ space, Newton's method only valid if Hessian $H$ is positively defined, 
\end{enumerate}
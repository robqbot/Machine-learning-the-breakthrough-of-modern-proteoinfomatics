...
AdaGrad adapts the initial learning rate $\mu$ for each epoch to give model parameter $\phi$ update as follows:
\begin{equation}
    \phi_{i+1} = \phi_{i} + \Delta \phi = \phi_i - \frac{\mu}{\delta + \sqrt{\tau_}} \odot \mathcal{g}
\end{equation}
where $\delta$ is a small stabilization constant with a magnitude of $10^{-7}$. 

...
\end{equation}
The model parameter $\phi$ is updated after each sample evaluation by $\phi_{i+1} = \phi_i - \mu g$. In a convex space, the gradient points towards minimum is decreasing after each training epoch, it is common practise to employ a decreased learning rate $\mu_i$ over each iteration. Major benefits by adopting vanishing gradient in SGD include: preventing overshooting the optimum due to large step size around minimum region and combating systematic noise associated with SGD. 

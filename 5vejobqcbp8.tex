...
\begin{equation}
    \mathcal{v} \approx \lambda \mathcal{v} - \mu \mathcal{H}_{\phi} [\frac{1}{m} \sum_{i=1}^m
    \mathcal{Obj}(\phi)]
\end{equation}
where $\lambda$ is a hyper-parameter with range $[0,1)$ and defines the proportion of last gra
\end{enumerate}

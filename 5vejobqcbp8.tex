...
\end{align}
where $H$ is the Hessian matrix for $Obj$ at $\phi_i$. As long as $\mathcal{Obj}$ is convex, model can be trained iteratively. For the non-convex $\mathcal{Obj}$ space, Newton's method only valid if Hessian $H$ is positively defined, it is considered dangerous as training can go into adverse direction. \citet{Levenberg_1944} and~\citet{Marquardt_1963} proposed Levenberg-Marquardt algorithm, which provides a regularization strategy to combat non-convex diver
\end{enumerate}
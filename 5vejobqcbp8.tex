...
\begin{equation}
    \mathrm{d}_{i+1} = \nabla_{\phi}\mathcal{Obj}(\phi) + \beta_{i+1}\mathrm{d}_i
\end{equation} 
where $\beta_{i+1}$ is a hyperparameter that adjust the contribution of previous search direction from previous iteration.  
\end{enumerate}
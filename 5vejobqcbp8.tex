...
Stochastic gradient with momentum can be denoted as: 
\begin{equation}
    \mathcal{v} = \lambda \mathcal{V} - \mu g \approx \lambda \mathcal{V} - \mu \mathcal{H}_{\phi} [\frac{1}{m} \sum_{i=1}^m
    \mathcal{Obj}(\phi)]
\end{equation}
where $\mathcal{V}$ is the average of the gradients $g = \mathcal{H}_{\phi} [\frac{1}{m} \sum_{i=1}^m
    \mathcal{Obj}(\phi)] $ across training set with training elements of $m$ $\lambda$ is a hyper-parameter with range $[0,1)$ and defines the weight of last gradient of the 
\end{enumerate}

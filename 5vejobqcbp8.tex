...
Optimisation in machine learning can be generalized as finding $\phi$ to make $\mathcal{Obj}_{\phi}(X)$ at any given input $X$ minimum. \\
For a multi-dimensional input $\mathcal{x}$, directional gradient at $\mathcal{x}$ in the direction $\mathcal{u}$ defined by $\mathcal{u}^T\bigtriangledown_xf(x)$ provides information of the changing direction of the 

...
Optimisation in machine learning can be generalized as finding $\phi$ to make $\mathcal{Obj}_{\phi}(X)$ at any given input $X$ minimum. For a multi-dimensional input $\mathcal{x}$, directional gradient at $\mathcal{x}$ in the direction $\mathcal{u}$ defined by $\mathcal{u}^T\bigtriangledown_xf(x)$. Directional gradient provides information about the changing direction of $\mathcal{Obj}_{\phi}(x)$, to improve the fitness of the model, we 

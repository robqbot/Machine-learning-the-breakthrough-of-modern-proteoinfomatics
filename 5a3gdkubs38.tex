...
\begin{equation}
    \mathcal{L} = \mathcal{D}_{KL}(p \parallel q) = \mathbf{H}_p(\mathrm{q}) - H(q) 
    = \frac {1}{n} \sum_i^n \mathrm{p} \times log \mathrm{q} + \frac{1}{n}\sum_{i=1}^n [\mathrm{p} \times log \mathrm{p}]
\end{equation}

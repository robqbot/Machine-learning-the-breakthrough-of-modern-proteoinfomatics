...
Likehood loss is commonly used in classification models. 
    \item Cross Entropy \\
Cross entropy is widely applicable in binary classification, which is extremely popular in visual pattern recognition application with convolutional neural network, which 
\begin{equation}
    MSE = \frac{1}{n}\sum_{i=1}^n (\hat{x}_i - x_i)^2
\end{equation}
\end{enumerate}

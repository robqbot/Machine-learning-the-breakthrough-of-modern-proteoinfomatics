...
KL divergence better known as relative entropy, is a probabilistic measurement on the divergence of predicted distribution to expected distribution. KL divergence can be interpreted as the combination of predictions entropy and the cross entropy between model prediction and training data, which has the form of:
\begin{equation}
    \mathcal{L} = \mathbf{H}(p) + \mathcal{D}_{KL}(p \parallel q) = \frac{1}{n}\sum_{i=1}^n [\hat{y}_i \cdot log(\hat{y_i})] -\frac{1}{n}\sum_{i=1}^n [y \cdot log\hat{y_i} +(1-y_i) \times log(1-\hat{y_i}) ]
\end{equation}
\end {enumerate}

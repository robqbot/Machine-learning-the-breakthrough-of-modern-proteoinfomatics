...
KL divergence better known as relative entropy, is a probabilistic measurement on the divergence of predicted distribution to expected distribution. KL divergence can be interpreted as the difference between cross entropy $H_p(q)$ and entropy of training data $H(q)$ which can take the form of:
\begin{equation}
    \mathcal{L} = \mathbf{H}(p) + \mathcal{D}_{KL}(p \parallel q) = \frac{1}{n}\sum_{i=1}^n [\hat{y}_i \cdot log(\hat{y}_i)] -\frac{1}{n}\sum_{i=1}^n [y \cdot log\hat{y_i}]
\end{equation}

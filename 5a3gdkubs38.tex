...
\begin{equation}
    E(\mathcal{I}) = \frac {1}{2n} \sum_x^n \parallel y(\mathcal{I}) - y^*(\mathcal{x}) \parallel^2 
\end{equation}

where $\mathcal{x}$ is the input vector, $y^*(\mathcal{x})$ is the actual output with given input vector $mathcal{x}$, $y(\mathcal{x})$ is the model predicted output with given $\mathcal{x}$. 
\par 

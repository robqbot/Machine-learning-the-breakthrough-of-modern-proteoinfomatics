...
Loss functions are used to measure the discrepancy between model prediction and training data. Loss functions broadly divided into two categories: classification and regression. As the cross entropy loss (also known as log loss) arguably the most popular loss function choice for classification models, Mean Square Error (MSE) is probably the most common regression loss function. 
\par 
Due to most of the modern neural networks adopt back propagation algorithm during training, the loss function therefore has to be at least 1\textdegree differentiable to allow error gradient to be calculated. The loss function can be generalized in the form:
...
\par 
We here discuss a few common loss functions that are used in modern machine learning models include:

...
\begin{equation}
    \mathcal{L} = \mathbf{H}(p) + \mathcal{D}_{KL}(p \parallel q) = \frac{1}{n}\sum_{i=1}^n [\hat{y}_i \cdot log(\hat{y_i})] -\frac{1}{n}\sum_{i=1}^n [y_i \times log\hat{y_i} +(1-y_i) \times log(1-\hat{y_i}) ]
\end{equation}
\end {enumerate}

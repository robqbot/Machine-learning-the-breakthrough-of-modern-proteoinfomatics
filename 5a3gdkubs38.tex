...
\begin{equation}
   p = \left\{ 
    \begin{array}{lr}
        log(\mathrm{prob}(y_i)) & y_i = 1~\text\text{(i.e.} yin positive class)  \\
        log(\mathrm{prob}(1-y_i))y_i & y_i = 0 (y_i in negative class)
    \end{array}\right.
    \left $y_i = 1$ when positive class
$y_i = 0$ when negative class
defined as:
\begin{equation}

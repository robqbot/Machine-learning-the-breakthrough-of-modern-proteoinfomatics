...
In linear regression models, model parameters can be learned is based on Bayesian estimation to learn model parameters as given by 
\begin{equation}
    \mathcal{\hat y} = \mathrm{X}\mathcal{w}
\end{equation}
where $\mathrm{X}$ is input vector, $\mathcal{w}$ is the parameter matrix. 

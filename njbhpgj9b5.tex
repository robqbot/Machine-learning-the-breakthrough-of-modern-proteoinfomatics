...
\end{equation}
where $\phi$ is model parameter matrix and $\psi$ is an sigmoid function. Solving for optimum parameter matrix $\phi$ is somewhat difficult, given that there is no closed-form solution to its optimum. A common approach is to search the parameter space by 

...
The optimum model parameter matrix $\phi$ can be approximated under Bayesian estimate when prior mean $\mu$ set to zero and covariance $\Lambda$ set to $\frac{1}{\alpha}\mathcal{I}$, while $\alpha$ is the learning rate. \citet{Goodfellow-et-al-2016} provided a detailed theoretical bases for Bayesian estimation based proba
In order to define posterior distribution for model parameter $\phi$, 

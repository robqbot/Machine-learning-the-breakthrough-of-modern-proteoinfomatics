...
where $x\in\mathbb{R}^d$, that can be either input vector $\upsilon$ or previous hidden layer's state vector $\mathcal{p}$. Both weight matrix $\mathcal{W}\in\mathbb{R}^{d,m,k}$ and bias matrix $\mathcal{B}\in\mathbb{R}^{m,k}$ are trainable. The third dimension of the weight matrix $\mathcal{W}$ corresponds to the connection state of neighbouring layer
\end{enumerate}
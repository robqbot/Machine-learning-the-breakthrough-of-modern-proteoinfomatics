...
$O_o = g( \sum_{j} \sum_{i} B2B1*W_{ij} * I_i)$ \\
where $I_i$ represents the input layer with i nodes, $O_o$ refers as output layer with o nodes, $W_{i}$ is the weight of each input node $I_i$ in representing hidden node $H_j$, $W_{j}$ is the weight of each hidden node $H_j$ in representing output node $O_o$
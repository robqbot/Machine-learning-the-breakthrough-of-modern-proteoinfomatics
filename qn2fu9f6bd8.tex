...
$O_i = g( \sum_{j} W_{j}(\sum_{i} W_{i} * I_i)_j$
where $I_i$ represents the input nodes, $W_{i}$ is the weight of each input node $I_i$ in representing hidden node $H_a$, $W_{j}$ is the weight of each hidden node $H_a$ in representing output node $O
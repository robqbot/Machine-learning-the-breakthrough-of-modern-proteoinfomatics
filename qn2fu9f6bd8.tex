...
where $x\in\mathbb{R}^d$, that can be either input vector $\upsilon$ or previous hidden layer's state vector $\mathcal{p}$. Both weight matrix $\mathcal{W}\in\mathbb{R}^{d\times m\times	k}$ and bias matrix $\mathcal{B}\in\mathbb{R}^{mk}$ are trainable. The third dimension of the weight matrix $\mathcal{W}$ corresponds to the connection state of neighbouring layers. 
\end{enumerate}
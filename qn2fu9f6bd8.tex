...
where $x\in\mathbb{R}^d$, that can be either input vector $\upsilon$ or previous hidden layer's state vector $\mathcal{p}$. Both weight matrix $\mathcal{W}\in\mathbb{R}^{d,m,k}$ and bias matrix $\mathcal{B}\in\mathbb{R}^{m,k}$ are tria
\end{enumerate}
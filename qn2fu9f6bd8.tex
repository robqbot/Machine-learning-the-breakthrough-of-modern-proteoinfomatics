...
However, as Figure shown, logistic function plateaus at either 0 or 1, which results nodes saturation around plateau and convergence gradients at both regions are approaching 0, which results an inefficient learning. Furthermore, logistic output $g(x) \geq 0$, this will govern the gradient descent always on the same side 
\item Softmax \\

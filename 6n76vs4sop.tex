...
Quantile loss is another commonly used loss function, which provides a confident envelope around prediction. Quantile loss is valuable in model predicting intervals rather than a specific value. One assumption that quantile loss based on is that the residuals between model prediction and targets 
\end{enumerate}